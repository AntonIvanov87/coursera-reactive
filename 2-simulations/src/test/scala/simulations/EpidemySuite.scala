  ...
  test("life cycle"){
    ...
    while(!personDied){
      ...
      if (infectedPerson.dead) {
        assert(infectedPerson.infected === true, "Dead person should stay infected")
        assert(infectedPerson.sick === true, "Dead person should stay sick")
        assert(infectedPerson.immune === false, "Dead person should not have immune")
        ...
        while(es.agenda.head.time < immuneTime) {
          assert(infectedPerson.infected === true, "Recovering person should stay infected")
          assert(infectedPerson.sick === true, "Recovering person should stay sick")
          assert(infectedPerson.immune === false, "Recovering person should not have immune")
          assert(infectedPerson.dead === false, "Recovering person should not be dead")
          es.next
        }
        ...
        while(es.agenda.head.time < healTime) {
          assert(infectedPerson.infected === true, "Recovered person should stay infected")
          assert(infectedPerson.sick === false, "Recovered person should not be sick")
          assert(infectedPerson.immune === true, "Recovered person should have immune")
          assert(infectedPerson.dead === false, "Recovered person should not be dead")
          es.next
        }
        assert(es.agenda.head.time === healTime)
        while(es.agenda.head.time == healTime) es.next

        assert(infectedPerson.sick === false, "Fully recovered person should not be sick")
        assert(infectedPerson.immune === false, "Fully recovered person should not have immune")
        assert(infectedPerson.dead === false, "Fully recovered person should not be dead")
