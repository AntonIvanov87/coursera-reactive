...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    // TO IMPLEMENT
    val searchTerms: Observable[String] = Observable("scala")

    // TO IMPLEMENT
    val suggestions: Observable[Try[List[String]]] = searchTerms.concatRecovered {
      wikiSuggestResponseStream(_).timedOut(1)
    }
    ...
    val suggestionSubscription: Subscription =  suggestions.observeOn(eventScheduler).subscribe(
      x => x match {
        case Success(suggests) => println("got suggestions " + suggests); suggestionList.listData = suggests.toSeq
        case Failure(ex) => println("failed to get suggestions: " + ex); status.text = ex.getLocalizedMessage
      }
    )

    // TO IMPLEMENT
    val selections: Observable[String] = button.clicks
      .map(_ => suggestionList.listData)
      .filter(_.nonEmpty)
      .map(_.head)

    // TO IMPLEMENT
    val pages: Observable[Try[String]] = selections.concatRecovered {
      wikiPageResponseStream(_).timedOut(1)
    }

    // TO IMPLEMENT
    val pageSubscription: Subscription = pages.observeOn(eventScheduler) subscribe {
      x => x match {
        case Success(page) => println("got page"); editorpane.text = page
        case Failure(ex) => println("failed to get page: " + ex); editorpane.text = ex.getLocalizedMessage
      }
    }
