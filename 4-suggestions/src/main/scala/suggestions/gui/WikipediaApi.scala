...
trait WikipediaApi {
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
    ...
    def timedOut(totalSec: Long): Observable[T] = {
      ...
      obs.takeWhile(_ => System.currentTimeMillis() - start < totalMs)
    ...
    def concatRecovered[S](requestMethod: T => Observable[S]): Observable[Try[S]] = {
      ...
      obs.recovered.flatMap({
        case Success(v) => requestMethod(v).recovered
        case Failure(e) =>
          val oe: Observable[S] = Observable.apply(e)
          oe.recovered
      })
