...
trait SwingApi {
  ...
  implicit class TextFieldOps(field: TextField) {
    ...
    /** Returns a stream of text field values entered in the given text field.
      ...
      */
    def textValues: Observable[String] = Observable.apply(
      observer => {
        val reaction: Reaction = {
          case e => observer.onNext(field.text)
        }
  ...
  implicit class ButtonOps(button: Button) {
     ...
     */
    def clicks: Observable[Button] = Observable.apply(
      observer => {
        val reaction: Reaction = {
          case e => observer.onNext(button)
        }
