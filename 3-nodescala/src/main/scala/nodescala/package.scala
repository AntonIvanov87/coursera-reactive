...
package object nodescala {
  ...
  implicit class FutureCompanionOps[T](val f: Future.type) extends AnyVal {
     ...
     */
    def all[T](fs: List[Future[T]]): Future[List[T]] = allWorkaround(fs)
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      ...
      val prom = Promise[T]()
      for (fut <- fs) fut onComplete prom.tryComplete
      prom.future
  ...
  private def allWorkaround[T](fs: List[Future[T]]): Future[List[T]] = async {

    var rest = fs
    val acc = scala.collection.mutable.ListBuffer[T]()
    while (rest != Nil) {
  ...
  object CancellationTokenSource {
    ...
    def apply(): CancellationTokenSource = new CancellationTokenSource {
      ...
      val cancellationToken: CancellationToken = new CancellationToken {
        def isCancelled: Boolean = prom.future.value != None
      }
