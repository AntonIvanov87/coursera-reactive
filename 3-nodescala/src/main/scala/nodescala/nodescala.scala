...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    Future.run()(cancelToken => {
      Future {
        while (!cancelToken.isCancelled) {
          for {
            (request, exchange) <- listener.nextRequest()
          } respond(exchange, cancelToken, handler(request))
        }
      }
    })
  }
...
object NodeScala {
  ...
  trait Listener {
    ...
    def nextRequest(): Future[(Request, Exchange)] = {
      val prom = Promise[(Request, Exchange)]()

      createContext((exchange: Exchange) => {
        prom.complete(
          Try((exchange.request, exchange))
        )
        removeContext()
      })
